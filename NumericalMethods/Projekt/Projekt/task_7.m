...
% The progress curve to be used
C = @(s) abs(0.8.*s + 0.2.*sin(9/2*pi*s));

figure(1)
plot(linspace(0, 1, 100), C(linspace(0, 1, 100)))
title('Progress curve C(s)')

% Number of equidistant segments
% Higher values gives slower but smoother animation
n = 20;

t_span_3 = zeros(n+1, 1);
...
for i = 1:n-1
    ...
    % Redefine function with new s_value
    arc_s = @(t) C(s)*tot_len - adapquad(arcfunc, 0, t, 1e-6);

    % Compute t*
    t_span_3(i+1) = newton(init_guess, 1e-4, arc_s, darc_s);
end
t_span_3(end) = 1;


time = linspace(0, 1, (size(t_span_3, 1)-1)*10);

% Create plot
figure(2)
plot(x(time), y(time), 'b-')
axis([-0.5 1.5 0 1.6])
title('animation with C(s)')
hold on

% Create the 'ball'
d = plot(x(0), y(0), 'r.', markersize=20);

% Start animation
for k = 1:size(t_span_3, 1) - 1

    % Create sub-set of t from t*n to t*n+1
    time = linspace(t_span_3(k), t_span_3(k+1), 10);
